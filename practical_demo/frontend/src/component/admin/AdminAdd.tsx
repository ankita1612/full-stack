...
function AdminAdd() {
  ...
  useEffect(() => {
    ...
    const fetchAdmin = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get(BACKEND_URL + `/api/admin/${id}`);
        Object.entries(data.data).forEach(([k, v]) =>
          setValue(k as keyof IAdmin, v),
        );
  ...
  const onSubmit = async (data: IAdmin) => {
    ...
    try {
      ...
      let res: any;
      if (mode == "add") {
        res = await axios.post(BACKEND_URL + "/api/admin", send_data);
      } else {
        res = await axios.put(BACKEND_URL + "/api/admin/" + id, send_data);
      }
